...
class Item(QtWidgets.QTreeWidgetItem):
    ...
    def __init__(self, *args):
        ...
        self._pixmapRectForIconType = None
    ...
    def dragProcess(self, isProcess):
        self._isDragProcess = isProcess
    ...
    def paint(self, painter, option, index):
        ...
        try:
            ...
            iconRect = self.paintIcon(painter, option, index)

            if index.column() == 0 and not self._isDragProcess:
                self.paintSelectionIndex(painter, option, index, iconRect)
    ...
    def paintSelectionIndex(self, painter, option, index, iconRect):
        isSelected = option.state & QtWidgets.QStyle.State_Selected
        if not isSelected:
            return
        selectedItems = self.itemsWidget().libraryWindow().getSelectedITemsExtended()
        if len(selectedItems) <= 1:
            return
        itemIndex = -1
        for i in range(0, len(selectedItems)):
            if selectedItems[i] == self:
                itemIndex = i+1
                break
        if itemIndex == -1:
            return
        ...
        visualRect = iconRect#self.visualRect(option)

        cWidth = min(35, visualRect.width())
        cPadding = 0
        cText = str(itemIndex)
        #cX = visualRect.center().x()# - float(cWidth / 2)
        #cY = visualRect.center().y()#visualRect.top() + cPadding
        cRect = visualRect#QtCore.QRect(cX, cY, cWidth, cWidth)
    ...
    def paintIcon(self, painter, option, index, align=None):
        ...
        self._pixmapRectForIconType = pixmapRect
        return pixmapRect
    ...
    def typeIconImage(self, size):
        ...
        icon.setColor(QtGui.QColor(50, 50, 50, 200))
    ...
    def typeIconRect(self, option):
        ...
        maxWidth = 30
        width = minWidth
        if self._pixmapRectForIconType:
            ratio = self._pixmapRectForIconType.width()/200.0
        if ratio < 0:
            width = minWidth
        elif ratio > 100:
            width = maxWidth
        ...
        rect = QtCore.QRect(x, y, width * self.dpi(), width * self.dpi())
    ...
    def paintTypeIcon(self, painter, option):
        ...
        icon = self.typeIconImage(rect.size())
        if icon:
