...
class LibraryItem(studiolibrary.widgets.Item):
    ...
    def createAction(cls, menu, libraryWindow):
        ...
        if cls.NAME:
            ...
            if not cls is AnimItem:
                ...
                icon = QtGui.QIcon(cls.CM_ICON_PATH)
                animMenu = QtWidgets.QMenu(menu)
                animMenu.setIcon(icon)
                icon = studioqt.Icon(animMenu.icon())
                icon.setColor(QtGui.QColor(255, 255, 255, 200))
                animMenu.setIcon(icon)
                animMenu.setTitle("Animation")

                icon = QtGui.QIcon(cls.CM_SL_ICON_PATH)
                callback = partial(cls.showSaveWidget, libraryWindow)
                action = QtWidgets.QAction(icon, "Studio Library", animMenu)
                action.triggered.connect(callback)
                icon = studioqt.Icon(action.icon())
                icon.setColor(QtGui.QColor(255, 255, 255, 200))
                action.setIcon(icon)
                animMenu.addAction(action)

                icon = QtGui.QIcon(cls.CM_CLIP_ICON_PATH)
                action = QtWidgets.QAction(icon, "Import Clips", animMenu)
                action.triggered.connect(libraryWindow.onImportAnimationClips)
                icon = studioqt.Icon(action.icon())
                icon.setColor(QtGui.QColor(255, 255, 255, 200))
                action.setIcon(icon)
                animMenu.addAction(action)

                icon = QtGui.QIcon(cls.CM_AA_ICON_PATH)
                action = QtWidgets.QAction(icon, "Anim Assistant", animMenu)
                action.triggered.connect(libraryWindow.onImportFromAnimAssist)
                icon = studioqt.Icon(action.icon())
                icon.setColor(QtGui.QColor(255, 255, 255, 200))
                action.setIcon(icon)
                animMenu.addAction(action)

                menu.addMenu(animMenu)
                return None
    ...
    @classmethod
    def getClipboardItems(cls):
        global clipboardItems
        return clipboardItems
    
    @classmethod
    def clearClipboardItems(cls):
        global clipboardItems
        clipboardItems = []

    @classmethod
    def getClipboardCopy(cls):
    ...
    def addCCPMenu(self, menu, items=None):
        iconColor = QtGui.QColor(255, 255, 255, 200)
        
        action = QtWidgets.QAction("Copy", menu)
        action.triggered.connect(self.onCopy)
        icon = studiolibrary.resource.icon("copy-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)

        action = QtWidgets.QAction("Cut", menu)
        action.triggered.connect(self.onCut)
        icon = studiolibrary.resource.icon("cut-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)
        ...
        action.setEnabled(len(clipboardItems) != 0)
        action.triggered.connect(self.onPaste)
        icon = studiolibrary.resource.icon("paste-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)
    ...
    def contextEditMenu(self, menu, items=None):
        ...
        icon = studiolibrary.resource.icon("rename-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)

        self.addCCPMenu(menu, items)

        action = QtWidgets.QAction("Copy to", menu)
        action.triggered.connect(self.onShowCopyToDialog)
        icon = studiolibrary.resource.icon("copy_to-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)

        action = QtWidgets.QAction("Move to", menu)
        action.triggered.connect(self.onShowMoveToDialog)
        icon = studiolibrary.resource.icon("move_to-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)
        ...
        icon = studiolibrary.resource.icon("copy_path-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)
        ...
        if self.libraryWindow():
            ...
            icon = studiolibrary.resource.icon("select_folder-menu-icon")
            icon.setColor(iconColor)
            action.setIcon(icon)
            menu.addAction(action)
        ...
        icon = studiolibrary.resource.icon("show_in_folder-menu-icon")
        icon.setColor(iconColor)
        action.setIcon(icon)
        menu.addAction(action)
    ...
    def onShowMoveToDialog(self):
        self.showMoveDialog(self.libraryWindow())
    
    def onShowCopyToDialog(self):
        self.showMoveDialog(self.libraryWindow(), copy=True)
    ...
    def createOverwriteMenu(self, menu):
        ...
        if not self.isReadOnly():
            ...
            icon = studiolibrary.resource.icon("overwrite-menu-icon")
            icon.setColor(iconColor)
            action.setIcon(icon)
            menu.addAction(action)
    ...
    def rename(self, dst, extension=None, force=False):
        """
        ...
        for libraryWindowInst in librarywindow.LibraryWindow.instances():
            if libraryWindowInst.currentItem() and libraryWindowInst.currentItem().path() == self.path():
                libraryWindowInst.clearPreviewWidget()
        ...
        dst = studiolibrary.renamePath(src, dst, force=force)
    ...
    def onCopy(self):
        global clipboardItems
        global clipboardCopy
        clipboardItems = self.getSelectedITems()
        clipboardCopy = True

    def getSelectedITems(self):
        return self.libraryWindow().getSelectedITemsExtended()
    ...
    def onPaste(self):
        ...
        for clipboardItem in clipboardItems:
            ...
            srcFileName = os.path.basename(clipboardItem.path())
            dstPath = os.path.join(dstPath, srcFileName)
            
            if clipboardCopy:
                clipboardItem.copy(dstPath)
            else:
                clipboardItem.move(dstPath)
                clipboardItem = []
        if not clipboardCopy:
            clipboardItem = []
    ...
    def showRenameDialog(self, parent=None):
        ...
        if button == QtWidgets.QDialogButtonBox.Ok:
            try:
                widget = self.libraryWindow().previewWidget()
                if widget and hasattr(widget, "stopPlay"):
                    widget.stopPlay()
    ...
    def showMoveDialog(self, parent=None, copy=False):
        """
        ...
        if dst:
            itemsToMove = self.getSelectedITems()
            self.libraryWindow().moveItems(itemsToMove, dst, copy)
    ...
    def showDeleteDialog(self):
        ...
        if button == QtWidgets.QDialogButtonBox.Yes:
            try:
                itemsToDelete = self.getSelectedITems()
                self.libraryWindow().moveItemsToTrash(itemsToDelete)
            except Exception as error:
