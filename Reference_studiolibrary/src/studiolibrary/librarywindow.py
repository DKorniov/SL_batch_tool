...
from studiolibrary.widgets.separatoraction import SeparatorAction
...
class PreviewFrame(QtWidgets.QScrollArea):
    pass
...
class LibraryWindow(QtWidgets.QWidget):
    ...
    def destroyInstance(name):
        """Delete library widget instance."""
        for widget in LibraryWindow.instances():
            if widget.name() == name:
                del LibraryWindow._instances[name]
                widget.destroy()
                return
    
    @staticmethod
    def renameInstance(name, newName):
        """Rename library widget instance."""
        for widget in LibraryWindow.instances():
            if widget.name() == name:
                LibraryWindow._instances[newName] = LibraryWindow._instances[name]
                del LibraryWindow._instances[name]
                return
    ...
    def __init__(self, parent=None, name="", path=""):
        ...
        self._previewFrame.setSizePolicy(QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding))
        self._previewFrame.setAlignment(QtCore.Qt.AlignTop)
        self._previewFrame.setWidgetResizable( True )
        ...
        self._itemsWidget.setLibraryWindow(self)
        ...
        self._sidebarWidget = self.SIDEBAR_WIDGET_CLASS(name, self)
        ...
        self._scrollWidget = QtWidgets.QFrame(self._previewFrame)
        self._scrollWidget.setContentsMargins(0, 0, 0, 0)
        self._scrollWidget.setSizePolicy(QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding))
        self._previewLayout = QtWidgets.QVBoxLayout(self._scrollWidget)
        self._previewFrame.setWidget(self._scrollWidget)
        self._previewLayout.setSpacing(0)
        self._previewLayout.setContentsMargins(0, 0, 0, 0)
        self._previewFrame.setMinimumWidth(5)
        self._previewFrame.setHorizontalScrollBarPolicy( QtCore.Qt.ScrollBarAlwaysOff )
        self._previewFrame.setVerticalScrollBarPolicy( QtCore.Qt.ScrollBarAsNeeded )
        ...
        itemsWidget.itemMovedTree.connect(self._itemMoved)
        itemsWidget.itemDroppedTree.connect(self._itemDropped)
    ...
    def _foldersMenuRequested(self, menu):
        ...
        """
        action = SeparatorAction("Library", menu)
        menu.addAction(action)

        addPathAction = QtWidgets.QAction("Add new...", menu)
        addPathAction.triggered.connect(self.onShowAddPathDialog)
        menu.addAction(addPathAction)

        action = QtWidgets.QAction("Edit", menu)
        action.triggered.connect(self.onShowEditLibraryDialog)
        menu.addAction(action)

        action = QtWidgets.QAction("Delete", menu)
        action.triggered.connect(self.onDeleteLibrary)
        menu.addAction(action)

        action = QtWidgets.QAction("Delete All", menu)
        action.triggered.connect(self.onDeleteAllLibraries)
        menu.addAction(action)

        action = QtWidgets.QAction("Set to (default)", menu)
        action.triggered.connect(self.onSetToDefaultLibrary)
        #action.setEnabled(studiolibrary.defaultLibraryName() != self.name())
        menu.addAction(action)
    
    def onImportAnimationClips(self):
        from studiolibrary.widgets.clips_importer_controller import ClipsImporterControler
        from studiolibrary.widgets.clips_importer_model import ClipsImporterModel
        from studiolibrary.widgets.clips_importer import Ui_Dialog
        self.controler = ClipsImporterControler(self)
        self.ui = Ui_Dialog()
        self.model = ClipsImporterModel(self.ui)
        self.ui.setupUi(self.controler)
        self.controler.setModel(self.model)
        self.controler.show()
    
    def onImportFromAnimAssist(self):
        from studiolibrary.widgets.anim_assist_importer import AnimAssistImporter
        AnimAssistImporter(self)
    ...
    def showWelcomeDialog(self):
        ...
        dialog.accepted.connect(self.showSetInitionalPathDialog)

    def showSetInitionalPathDialog(self):
        """Show the settings dialog."""
        accentColor = self.theme().accentColor().toString()
        backgroundColor = self.theme().backgroundColor().toString()

        form = {
            "title": "Set library data path",
            "layout": "vertical",
            "schema": [
                {"name": "name", "type": "string", "default": ""},
                {"name": "path", "type": "path", "value": ""},
            ],
            "validator": self.newLibrarySettingsValidator,
            "accepted": self.newLibrarySettingsAccepted,
        }

        self._settingsWidget = studiolibrary.widgets.FormDialog(form=form)
        self._settingsWidget.setObjectName("newLibraryDialog")
        self._settingsWidget.acceptButton().setText("Set")
        self._settingsWidget.acceptButton().setEnabled(False)
        self._settingsWidget.rejectButton().setEnabled(False)
    ...
    def newLibrarySettingsValidator(self, **kwargs):
        fields = []
    ...
    def newLibrarySettingsAccepted(self, **kwargs):
        """
        ...
        if len(self.path()) == 0:
            studiolibrary.addLibrary(name, path)
        else:
            studiolibrary.cloneLibrary(self.name(), name, path)
        studiolibrary.reload()
        studiolibrary.main(name=name, path=path)
    
    def editLibrarySettingsAccepted(self, **kwargs):
        """
        Called when the user has accepted the changes in the settings dialog.
        ...
        if self.name() != name:
            studiolibrary.renameLibrary(self.name(), name)
            LibraryWindow.renameInstance(self.name(), name)
            self._name = name
            self.updateWindowTitle()
        if self.path() != path:
            studiolibrary.removeLibrary(self.name())
            LibraryWindow.destroyInstance(self.name())
            studiolibrary.addLibrary(self.name(), path)
            LibraryWindow.instance(name=self.name()).show()

    def onShowAddPathDialog(self):
        self.showAddPathDialog("Add new library", "Add", self.newLibrarySettingsAccepted, "", "")
    
    def onShowEditLibraryDialog(self):
        self.showAddPathDialog("Edit library settings", "Save", self.editLibrarySettingsAccepted, self.name(), self.path())

    def showAddPathDialog(self, title, okButtonTitle, callback, defaultName, defaultPath):
        """Show the settings dialog."""
        accentColor = self.theme().accentColor().toString()
        backgroundColor = self.theme().backgroundColor().toString()
        ...
        form = {
            ...
            "schema": [
                ...
                {"name": "path", "type": "path", "value": defaultPath},
            ],
            "validator": self.newLibrarySettingsValidator,
            "accepted": callback,
        }
        ...
        self._settingsWidget = studiolibrary.widgets.FormDialog(form=form)
        self._settingsWidget.setObjectName("newLibraryDialog")
        self._settingsWidget.acceptButton().setText(okButtonTitle)
    ...
    def onDeleteLibrary(self):
        result = self.showQuestionDialog("", "Are you sure you want to delete this library?", QtWidgets.QDialogButtonBox.Yes | QtWidgets.QDialogButtonBox.No)
        if result == QtWidgets.QDialogButtonBox.Yes:
            studiolibrary.removeLibrary(self.name())
            LibraryWindow.destroyInstance(self.name())
    
    def onDeleteAllLibraries(self):
        result = self.showQuestionDialog("", "Are you sure you want to delete all libraries?", QtWidgets.QDialogButtonBox.Yes | QtWidgets.QDialogButtonBox.No)
        if result == QtWidgets.QDialogButtonBox.Yes:
            studiolibrary.setLibraries([])
            LibraryWindow.destroyInstances()

    def onSetToDefaultLibrary(self):
        studiolibrary.setDefaultLibrary(self.name())
    ...
    def getSelectedITemsExtended(self):
    ...
    def settingsAccepted(self, **kwargs):
        ...
        if path and path != self.path():
            self.setPath(path)
        path = kwargs.get("Ffmpeg exe path")
        if path and path != self.ffmpegPath():
    ...
    def showSettingDialog(self):
        ...
        form = {
            ...
            "schema": [
                ...
                {"name": "Ffmpeg exe path", "type": "path", "value": self.ffmpegPath()},
                {
                    "name": "accentColor",
                    "type": "color",
                    "value": accentColor,
    ...
    def createSettingsMenu(self):
        ...
        :rtype: studioqt.Menu
        """
        iconColor = QtGui.QColor(255, 255, 255, 200)
        menu = studioqt.Menu("", self)
        menu.setTitle("Settings      ")
        icon = studiolibrary.resource.icon("settings-menu-icon")
        icon.setColor(iconColor)
        menu.setIcon(icon)
    ...
    def onClearTrash(self):
        result = self.showQuestionDialog("", "Are you sure?", QtWidgets.QDialogButtonBox.Yes | QtWidgets.QDialogButtonBox.No)
        if result == QtWidgets.QDialogButtonBox.No:
            return
        import shutil
        folder = self.trashPath()
    ...
    def createItemContextMenu(self, items):
        ...
        if self.trashEnabled():
            ...
            for selectedItem in items:
                if trashPath in selectedItem.path():
                    action = QtWidgets.QAction("Clear", menu)
                    action.triggered.connect(self.onClearTrash)
                    menu.addAction(action)
                    return menu
        ...
        if not self.isLocked():
            ...
            if item:
                ...
                icon = studiolibrary.resource.icon("edit-menu-icon")
                icon.setColor(iconColor)
                ...
                if self.trashEnabled():
                    ...
                    callback = partial(self.showMoveItemsToTrashDialog, self.getSelectedITemsExtended())

                    action = QtWidgets.QAction("Delete", editMenu)
                    action.setEnabled(not self.isTrashSelected())
                    action.triggered.connect(callback)
                    icon = studiolibrary.resource.icon("trash-menu-icon")
                    icon.setColor(iconColor)
                    action.setIcon(icon)
                    editMenu.addAction(action)
            else:
                menu.addSeparator()
                items = studiolibrary.LibraryItem.getClipboardItems()
                action = QtWidgets.QAction("Paste", menu)
                action.setEnabled(len(items) != 0)
    ...
    def onPaste(self):
        items = studiolibrary.LibraryItem.getClipboardItems()
        self.moveItems(items, self.selectedFolderPath(), studiolibrary.LibraryItem.getClipboardCopy())
        if not studiolibrary.LibraryItem.getClipboardCopy():
            studiolibrary.LibraryItem.clearClipboardItems()
    ...
    def moveItems(self, items, dst, copy=False, force=False, resetSettings=True):
        """
        ...
        mbTitle = "Do you want to overwrite?"
        if resetSettings:
        ...
        for item in items:
            try:
                ...
                if len(error.args) > 0 and ("existing path" in error.args[0]):
                    result = self.showQuestionDialog(mbTitle, path, QtWidgets.QDialogButtonBox.Yes | QtWidgets.QDialogButtonBox.No, enableDontShowCheckBox=True)
                    if result == QtWidgets.QDialogButtonBox.Yes:
                        studiolibrary.removePath(path)
                        self.moveItems([item], dst, copy, force, resetSettings=False)
                else:
                    self.showExceptionDialog("Move Error", error)
                    raise
        self.library().blockSignals(False)

        for libraryWindowInst in LibraryWindow.instances():
            libraryWindowInst.sync()
    ...
    def setPreviewWidgetFromItem(self, item, force=False):
        ...
        if item and self._previewWidgetVisible:
            self.closePreviewWidget()
            try:
    ...
    def setPreviewWidget(self, widget):
        ...
        if self._previewWidget == widget:
            ...
            if self._previewWidget:
                ...
                self._previewLayout.addWidget(self._previewWidget)
    ...
    def closePreviewWidget(self):
        ...
        :rtype: None
        """
        layout = self._previewLayout#self._previewFrame.layout()
    ...
    def settings(self):
        ...
        settings["ffmpegPath"] = self.ffmpegPath()
    ...
    def showQuestionDialog(self, title, text, buttons=None, enableDontShowCheckBox=False):
        """
        ...
        return studiolibrary.widgets.MessageBox.question(self, title, text, buttons=buttons, enableDontShowCheckBox=enableDontShowCheckBox)
    ...
    def updateWindowTitle(self):
        ...
        self.sidebarWidget().updateTitle(self.name())
