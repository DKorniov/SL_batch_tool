...
def cloneLibrary(cloneName, name, path, **settings):
    """
    Add a new library with the given name, path and settings.
    ...
    libraries = readSettings()
    if len(cloneName) > 0:
        libraries[name] = libraries[cloneName]
    libraries.setdefault(name, {})
    libraries[name]["path"] = path
...
def setDefaultLibrary(name):
    libraries = readSettings()
    for libName in libraries:
        libraries[libName]["default"] = False
    if libraries and libraries[name]:
        libraries[name]["default"] = True

        saveSettings(libraries)

def defaultLibraryName():
    libraries = readSettings()
    for libName in libraries:
        if libraries[libName]["default"]:
            return libName
    return ''
...
def renameLibrary(name, newName):
    libraries = readSettings()
    if name in libraries:
        libraries[newName] = libraries[name]
        del libraries[name]
    saveSettings(libraries)

def isLibraryExists(name):
    libraries = readSettings()
...
def findItems(path, depth=50, **kwargs):
    """
...
def findItemsInFolders(folders, depth=50, **kwargs):
    """
...
def saveJson(path, data):
    ...
    data = collections.OrderedDict(sorted(data.items(), key=lambda t: t[0].lower()))
...
def absPath(data, start, depth=50):
    """
...
def walkup(path, match=None, depth=50, sep="/"):
    """
    :type path: str
    :type match: func
    :type depth: int
