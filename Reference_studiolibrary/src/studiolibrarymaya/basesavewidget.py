...
try:
    import mutils
    import mutils.gui
    from mutils.gui.videosettingsdialog import ShowVideoSettings
    from mutils.gui.videosettingsdialog import InitVideoSettings
    import maya.cmds
...
class BaseSaveWidget(QtWidgets.QWidget):
    ...
    def __init__(self, item, parent=None):
        ...
        widget.ui.menuButton.clicked.connect(self.onCloseButton)
    ...
    def onCloseButton(self):
        if self.item():
            self.item().libraryWindow().clearPreviewWidget()
        else:
            self.parent.clearPreviewWidget()
    ...
    def createTitleWidget(self):
        ...
        menuButton.setIcon(QtGui.QIcon(iconPath));
    ...
    def createSequenceWidget(self):
        ...
        self.ui.thumbnailButton = studiolibrary.widgets.ImageSequenceWidget(self, item=self._item)
        ...
        if self._item.needMovie():
            path = studiolibrary.resource.get("icons", "video-create-icon.svg")
            self.ui.thumbnailButton.addAction(
                path,
                "Capture new video",
                "Capture new video",
                self.thumbnailCaptureVideo
            )

            path = studiolibrary.resource.get("icons", "video-settings-icon.svg")
            self.ui.thumbnailButton.addAction(
                path,
                "Show Capture Video window",
                "Show Capture Video window",
                self.showCaptureWindowVideo
            )

        path = studiolibrary.resource.get("icons", "camera-create-icon.svg")
        self.ui.thumbnailButton.addAction(
            path,
            "Capture new image",
            "Capture new image",
        ...
        path = studiolibrary.resource.get("icons", "camera-setting-icon.svg")
        self.ui.thumbnailButton.addAction(
            path,
            "Show Capture window",
            "Show Capture window",
        ...
        path = studiolibrary.resource.get("icons", "import-image-icon.svg")
        self.ui.thumbnailButton.addAction(
            path,
        ...
        path = studiolibrary.resource.get("icons", "settings-icon.svg")
        self.ui.thumbnailButton.addAction(
            path,
            "Capture settings",
            "Capture settings",
            self.showCaptureSettings
        )
    ...
    def setItem(self, item):
        ...
        if schema:
            ...
            if item.path() and len(item.path()) > 0 and item.transferObject():
                comment = item.transferObject().description()
                formWidget.setValues({"comment": comment})
    ...
    def close(self):
        ...
        if self.ui and self.ui.thumbnailButton:
    ...
    def showCaptureWindowVideo(self):
        """Show the capture window for framing."""
        self.thumbnailCaptureVideo(show=True)
    ...
    def doThumbnailCapture(self, show=False, video=False):
        ...
        BaseSaveWidget.makeThumbnail(show, startFrame, endFrame, step, video, self._capturedCallback, self.ui.thumbnailButton, self.libraryWindow())

    @staticmethod
    def makeThumbnail(show, startFrame, endFrame, step, video, capturedCallback, delegate, libraryWindow, doExec=False, modelPanel=None):
        try:
            compression = "jpg"
            if maya.cmds.optionVar(exists="VideoSettingsImageFormat"):
                ...
                if imageFormat == 1:
                    compression = 'png'
            path = studiolibrary.tempPath("sequence", "thumbnail." + compression)
            if doExec:
                mutils.gui.thumbnailCaptureExec(
                    show=show,
                    path=path,
                    ...
                    video=video,
                    delegate=delegate,
                    beforeCaprtureCallback='stopPlay',
            ...
            studiolibrary.widgets.MessageBox.critical(libraryWindow, title, str(e))
            raise

    def thumbnailCaptureVideo(self, show=False):
        self.doThumbnailCapture(show=show, video=True)

    def thumbnailCapture(self, show=False):
        self.doThumbnailCapture(show=show, video=False)

    def showThumbnailCaptureDialog(self):
        self.doShowThumbnailCaptureDialog(video=False)
    
    def showThumbnailCaptureDialogVideo(self):
        self.doShowThumbnailCaptureDialog(video=True)

    def doShowThumbnailCaptureDialog(self, video=False):
        """
        ...
        if button == QtWidgets.QDialogButtonBox.Yes:
            self.doThumbnailCapture(video=video)
    ...
    def accept(self):
        ...
        try:
            ...
            extension = ""
            if self.item() and len(self.item().path()) > 0:
                filename, extension = os.path.splitext(os.path.basename(self.item().path()))
            ...
            BaseSaveWidget.fixThumbnail(thumbnail)

            print("replace thumb: " + thumbnail)
            ...
            raise
    @staticmethod
    def fixThumbnail(thumbnail):
        replacePart = "0000"
        if replacePart in thumbnail:
            dir = os.path.dirname(thumbnail)
            ext = os.path.splitext(thumbnail)[1]
            dst = os.path.join(dir, "thumbnail") + ext
            shutil.copyfile(thumbnail, dst)
            thumbnail = dst

        print("replace thumb: " + thumbnail)

    @staticmethod
    def saveNewItem(path, tumbnailPath, item, sequencePath, libraryWindow, **kwargs):
        try:
            BaseSaveWidget.doSave(path, tumbnailPath, item, sequencePath, **kwargs)
    ...
    def save(self, path, thumbnail):
        ...
        self.ui.thumbnailButton.stopPlay()
        item = self.item()
        BaseSaveWidget.doSave(path, thumbnail, item, sequencePath, **kwargs)
        self.close()
    
    @staticmethod
    def doSave(path, thumbnail, item, sequencePath, **kwargs):
        print("Save item, objects:" + str(kwargs['objects']))
        item.setPath(path)
        item.safeSave(
            thumbnail=thumbnail,
