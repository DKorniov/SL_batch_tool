...
from studiolibrarymaya.ApplyAllMessageBoxController import ShowApplyAllMessageBoxDialog
...
class BaseLoadWidget(QtWidgets.QWidget):
    ...
    def __init__(self, item, parent=None):
        ...
        widget.ui.closeButton.clicked.connect(self.onCloseButton)
        ...
        self._thumbnailGroupBox = groupBox

        # Create the thumbnail widget and set the image
        self.ui.thumbnailButton = studiolibrary.widgets.ImageSequenceWidget(self, item=self._item, groupWidget=groupBox)
        ...
        self._pic = None
        if self._formWidget.widget("Help ImagePic"):
            self._pic = self._formWidget.widget("Help ImagePic").widget()
            self._pic.setScaledContents(True)
            self._pic.setAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignCenter)
            self._pic.setSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
            self._formWidget.widget("Help Image").widget().stateChanged.connect(self.onUpdatePic)
            self._formWidget.widget("connect").widget().stateChanged.connect(self.onUpdatePic)
            self._formWidget.widget("option").widget().currentIndexChanged.connect(self.onUpdatePic)
            self.onUpdatePic()
    ...
    def onUpdatePic(self):
        if self._pic:
            isVisible = self._formWidget.value("Help Image")
            option = self._formWidget.value("option")
            connect = self._formWidget.value("connect")
            iconName = option
            if option == "replace all":
                iconName = "replaceCompletely"
            elif option == "delete all keys":
                iconName = "delete_all_keys"
            elif connect:
                iconName += "Connect"
            if isVisible:
                iconName += ".png"
                iconPath = os.path.join(studiolibrary.resource.get("icons"), iconName)
                pixmap = QtGui.QPixmap(iconPath)
                pixmap = pixmap.scaled(200, 169, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
                self._pic.setPixmap(pixmap)
            self._pic.setVisible(isVisible)

    def minimumSizeHint(self):
        minHeight = 120
        if self._thumbnailGroupBox.isChecked() and self.ui.thumbnailButton:
            minHeight += self.ui.thumbnailButton.getSize().height()
        if self._formWidget.value("infoGroup"):
            minHeight += 200
        if self._formWidget.value("commentGroup") and self._formWidget.value("commentGroup"):
            minHeight += 80
        if self._formWidget.value("namespaceGroup"):
            minHeight += 100
        if self._formWidget.widget("optionsGroup") and self._formWidget.value("optionsGroup"):
            minHeight += 200
        if self._pic and self._formWidget.value("Help Image"):
            minHeight += 169
        return QtCore.QSize(160, minHeight)

    def resizeEvent(self, event):
        print("resizeEvent(self, event)")
    ...
    def createTitleWidget(self):
        ...
        iconPath = studiolibrary.resource.get("icons", "close-red-icon.svg")
        closeButton.setIcon(QtGui.QIcon(iconPath));
        closeButton.setObjectName("closeButton")
        titleWidget.ui.closeButton = closeButton
    ...
    def stopPlay(self):
        if self.ui.thumbnailButton and self.ui.thumbnailButton.isVideoPlaying():
            self.ui.thumbnailButton.stopPlay()
    ...
    def accept(self):
        ...
        selectedItems = self.item().libraryWindow().getSelectedITemsExtended()
        if len(selectedItems) == 0:
            return
        if len(selectedItems) == 1:
            self.item().loadFromCurrentValues()
        else:
            itemType = type(selectedItems[-1])
            for item in selectedItems:
                if type(item) != itemType:
                    print("Selected item is not animation")
                    return
            dialog = ShowApplyAllMessageBoxDialog(parent=self.item().libraryWindow())
            dialog.windowClosed.connect(self.onApplyAllClosed)
    
    def onApplyAllClosed(self, animPlaceOnTimelineIndex):
        if animPlaceOnTimelineIndex >= 0:
            print("do " + str(animPlaceOnTimelineIndex))
            mbTitle = Animation.getMessageBoxApplyAllTitle()
            MessageBox.resetEnableDontShowCheckBox(mbTitle)
            selectedItems = self.item().libraryWindow().getSelectedITemsExtended()
            if animPlaceOnTimelineIndex == 0:
                self.item().setCurrentLoadValue("currentTime", False)
            isCurrentTimeFrame = self.item().currentLoadValue("currentTime")
            startFrame = int(maya.cmds.currentTime(query=True)) if isCurrentTimeFrame else 0
            initialFrame = startFrame
            endFrame = startFrame
            for itemIndex in range(0, len(selectedItems)):
                item = selectedItems[itemIndex]
                
                item.setCurrentLoadValue("currentTime", False)
                item.setCurrentLoadValue("useAnimAssist", self.item().currentLoadValue("useAnimAssist"))
                item.setCurrentLoadValue("setTimeRange", self.item().currentLoadValue("setTimeRange"))
                item.setCurrentLoadValue("connect", self.item().currentLoadValue("connect"))
                item.setCurrentLoadValue("option", self.item().currentLoadValue("option"))

                item.setCurrentLoadValue("apply_index", itemIndex)
                if animPlaceOnTimelineIndex > 0:
                    item.setCurrentLoadValue("start_frame_force", startFrame)
                item.loadFromCurrentValues()
                item.setCurrentLoadValue("apply_index", None)
                itemStart = item.transferObject().startFrame()
                itemEnd = item.transferObject().endFrame()
                if animPlaceOnTimelineIndex > 0:
                    item.setCurrentLoadValue("start_frame_force", None)
                    if animPlaceOnTimelineIndex == 1:
                        startFrame += itemEnd - itemStart + 1
                        endFrame = startFrame -1
                    elif animPlaceOnTimelineIndex == 2:
                        step = 100
                    ...
                    if itemIndex == 0:
                        initialFrame = itemStart
                    initialFrame = min(initialFrame, itemStart)
                    endFrame = max(endFrame, itemEnd)
                
            if self.item().currentLoadValue("setTimeRange"):
                    maya.cmds.playbackOptions(minTime=initialFrame, maxTime=endFrame)
