...
class BaseItem(studiolibrary.LibraryItem):
    ...
    def setCurrentLoadValue(self, name, value):
        if value == None:
            del self._currentLoadValues[name]
        else:
            self._currentLoadValues[name] = value
    ...
    def loadSchema(self):
        ...
        if self.transferObject():
            metaData = self.transferObject().metadata()
            refs = metaData.get("references")
            for ref in refs:
                if len(refFilesStr) > 0:
                    refFilesStr += ', '
                refFilesStr += os.path.basename(ref.get("filename"))
                ...
                if len(ns) > 0 and ns != ':':
                    ...
                    namespacesStr += ns
        if len(refFilesStr) == 0:
            refFilesStr = 'No references'
        if len(namespacesStr) == 0:
            namespacesStr = 'No namespaces'
    ...
    def save(self, thumbnail="", **kwargs):
        ...
        if thumbnail:
            ...
            shutil.move(thumbnail, self.path() + "/" + basename)
