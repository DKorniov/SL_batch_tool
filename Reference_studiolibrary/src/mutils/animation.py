...
import mutils
import mutils.gui
from studiolibrary.widgets.anim_assist_helper import AnimAssistHelper, AssistAnimParams
...
def saveAnim(
    ...
    if iconPath and os.path.exists(iconPath):
        name, compression = os.path.splitext(iconPath)
        shutil.copyfile(iconPath, path + "/thumbnail" + compression)
...
class Animation(mutils.Pose):
    ...
    def load(
        ...
        :type timeRange: bool or None
        :type useAnimAssist: bool or None
        """
        logger.info(u'Loading: {0}'.format(self.path()))

        connect = bool(connect)  # Make false if connect is None

        if (option == PasteOption.DeleteAllKeys or option == PasteOption.ReplaceAll) and (applyIndex == 0 or applyIndex == None):
            maya.cmds.delete(all=1, channels=1)
            AnimAssistHelper.setAnimAssistAnims([])
        if option == PasteOption.DeleteAllKeys:
            option = PasteOption.Insert
        ...
        logger.debug("Animation.load(objects=%s, option=%s, namespaces=%s, srcTime=%s, currentTime=%s, timeRange=%s, useAnimAssist=%s)" %
                     (len(objects), str(option), str(namespaces), str(sourceTime), str(currentTime), str(timeRange), str(useAnimAssist)))
        ...
        try:
            ...
            if currentTime and startFrame is None:
                startFrame = int(maya.cmds.currentTime(query=True))
            if startFrameForce != None:
                startFrame = startFrameForce
            
            realStart = 0
            realEnd = 0
            if timeRange or useAnimAssist:
                srcStart, srcEnd = sourceTime
                realStart = startFrame
                if realStart == None:
                    realStart = srcStart
                realEnd = realStart+(srcEnd-srcStart)
            
            if useAnimAssist:
                mbTitle = Animation.getMessageBoxApplyAllTitle()
                assistAnims = AnimAssistHelper.getAnimAssistAnims()
                found = False
                animName = os.path.splitext(os.path.basename(self.path()))[0]
                for anim in assistAnims:
                    if anim.name() == animName:
                        name, button = MessageBox.input(
                            list(LibraryWindow.instances())[-1],
                            mbTitle,
                        ...
                        if name != animName:
                            ...
                            found = True
                            anim.setBegin(realStart)
                            anim.setEnd(realEnd)
                        break
                if not found:
                    assistAnims.append(AssistAnimParams(animName, realStart, realEnd))
                AnimAssistHelper.setAnimAssistAnims(assistAnims)
            
            if timeRange:
                maya.cmds.playbackOptions(minTime=realStart, maxTime=realEnd)
    ...
    @staticmethod
    def getMessageBoxApplyAllTitle():
        return "Overwrite in Anim Assistant"